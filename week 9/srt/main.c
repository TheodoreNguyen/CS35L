			   ...
			   int recursion_level, int recursion_limit )
{
    ...
    int child_dir;
    for( child_dir=0; child_dir<6; ++child_dir )
    {
...
void* ThreadFunction(void *arg)
{
    ...
    for( px=(tID*(width/nthreads)); px<((tID+1)*(width/nthreads)); ++px )
    {
        ...
        for( py=0; py<height; ++py )
        {
            ...
            for( xs=-halfSamples; xs<=halfSamples; ++xs ) //the inner 2 loops is anti-aliasing
            {
                for( ys=-halfSamples; ys<=halfSamples; ++ys )
                {
                    ...
                    sub( pixel_ray.dir, pixel_target, camera_pos ); //subtracts 2 vectors and stores in .dir
                ...
                /* trace the ray from the camera that
                    ...
                    trace( &scene, sample_color, &pixel_ray, 0 ); //what your suposed to render, the vector it has to trace, once it returns, the color result will be stored in the sample_color struct
            ...
            int offset = py*3;
            scaled_color[px][offset] = gamma( pixel_color[0] ) * max_color;
            scaled_color[px][offset+1] = gamma( pixel_color[1] ) * max_color;
            scaled_color[px][offset+2] = gamma( pixel_color[2] ) * max_color;

            /* enforce caps, replace with real gamma */
            int i;
            for( i=offset; i<(offset+3); i++)
                scaled_color[px][i] = max( min(scaled_color[px][i], 255), 0);
...
main( int argc, char **argv )
{
	...
	{
             printf( "%.0f %.0f %.0f\n", scaled_color[k][i], scaled_color[k][i+1], scaled_color[k][i+2] );	
	}
         printf( "\n" );	
    }
